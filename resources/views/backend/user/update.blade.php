            ...
            <div class="page-content container-fluid">
                <div class="row">
                    <div class="col-md-12 col-lg-12">
                        <div class="card">
                            <div class="card-body">
                                {{ Form::open(['route' => 'update-user', 'method' => 'post','class'=>'form-horizontal','id'=>'form', 'enctype'=>'multipart/form-data']) }}
                                {{ Form::token() }}
                                    <div class="form-group row">
                                        <label for="fname" class="col-sm-3 text-right control-label col-form-label">Nama user</label>
                                        <div class="col-sm-9">
                                        {{ Form::text('nama', @$user->nama, [ 'class'=>'form-control input-sm form-control-line', 'id'=>'', 'placeholder'=>'Masukkan nama user...', 'required']) }} 
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-9">
                                        {{ Form::email('email', @$user->email, [ 'class'=>'form-control input-sm form-control-line', 'id'=>'', 'placeholder'=>'Masukkan email...', 'required']) }} 
                                        </div>
                                    </div>
                                    <div class="form-group row">
                                        <label for="fname" class="col-sm-3 text-right control-label col-form-label">NIP</label>
                                        <div class="col-sm-9">
                                        {{ Form::text('nip', @$user->nip, [ 'class'=>'form-control input-sm form-control-line', 'id'=>'', 'placeholder'=>'Masukkan nip...', 'required']) }} 
                                        </div>
                                    </div>
                                    <div class="form-group row">
                                        <label for="fname" class="col-sm-3 text-right control-label col-form-label">No. HP</label>
                                        <div class="col-sm-9">
                                        {{ Form::number('no_hp', @$user->no_hp, [ 'class'=>'form-control input-sm form-control-line', 'id'=>'', 'placeholder'=>'Masukkan nomor hp...', 'required']) }} 
                                        </div>
                                    </div>
                                    <div class="form-group row">
                                        <label for="fname" class="col-sm-3 text-right control-label col-form-label">Alamat</label>
                                        <div class="col-sm-9">
                                        {{ Form::textarea('alamat', @$user->alamat, [ 'class'=>'form-control input-sm form-control-line', 'rows'=>'5', 'placeholder'=>'Masukkan alamat...', 'required']) }} 
                                        </div>
                                    </div>
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-9">
                                            <select class="form-control input-sm form-control-line" name="id_level" required>
                                                ...
                                                @foreach(@$level as $lv)
                                                <option value="{{ @$lv->id }}" @if(@$user->id_level == @$lv->id) selected @endif>{{ @$lv->level }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
